...
def exploration2(df):
    df_info = pd.DataFrame(df.dtypes).T.rename(index={0:"Type des données"})
    ...
    df_info = df_info.append(pd.DataFrame(df.isna().sum()/df.shape[0]*100).T.rename(index={0:"NaN (%)"}))
    df_info = df_info.append(pd.DataFrame(df.nunique()).T.rename(index={0:"Nombre de valeurs uniques"}))
    df_info = df_info.append(pd.DataFrame(df.nunique()/df.shape[0]*100).T.rename(index={0:"Valeurs uniques (%)"}))
    df_info = df_info.T
    return df_info
